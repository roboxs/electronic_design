...
float single_pid_calculate(pid_t * pid_cal)
{
	if(pid_cal->pid_mode == POSITION_PID)
	{
		...
		/*积分项改进*/
		if( pid_cal->err[NOW] == 0)//不进行积分分离
		{
			pid_cal->iout += pid_cal->i * pid_cal->err[NOW];
		}
		else
		{
			if(pid_cal->err[NOW] < pid_cal->integral.err_min)//在区域中进行积分
			{
				pid_cal->iout += pid_cal->i * pid_cal->err[NOW];
			}
			else
		...
		pid_cal->pos_out = pid_cal->pout + pid_cal->iout + pid_cal->dout[NOW];
...
float pid_calculate(pid_t * pidInner,pid_t * pidOuter)
{
    if(pidInner->pid_mode == POSITION_PID) //位置式PID
    {
		...
		if(pidOuter->deadband != RESET)		dead_limit(&(pidOuter->err[NOW]),pidOuter->deadband);//死区限制
		//比例项
...
//		pidOuter->dout_lpf = Control_Device_LPF(pidOuter->dout[NOW],&pidOuter->bufferdata,&Control_Device_Div_LPF_Parameter);//20hz巴特沃斯低通滤波
		//积分限幅
		abs_limit(&(pidOuter->iout),pidOuter->IntegralLimit);
//		pidOuter->pos_out = (pidOuter->pout +   pidOuter->iout + pidOuter->dout[0]);
		...
		{
			if(ABS(pidOuter->err[NOW]) < pidOuter->integral.err_min) 
				pidOuter->pos_out = (pidOuter->pout +  pidOuter->iout + pidOuter->dout[NOW]);
			else 
				pidOuter->pos_out = (pidOuter->pout +  pidOuter->dout[NOW]);
		}
		//总输出限幅
