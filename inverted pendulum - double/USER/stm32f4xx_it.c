...
void TIM4_IRQHandler()
{
    ...
    {
         if(!(TIM4->CR1>>4 & 0x01))
            g_invert_control.g_encoder_data.round_cnt++;
        else if((TIM4->CR1>>4 & 0x01))
            g_invert_control.g_encoder_data.round_cnt--;
...
static MotoMeasure_t motor_6020;

const MotoMeasure_t * get_motor_measure_point(void)
{
	return &motor_6020;
}
...
void CAN1_RX0_IRQHandler(void)
{
	...
	switch(g_can1_receive_str.StdId)
	{
		case 0x205:
		{
			encoder_data_handler(&motor_6020, &g_can1_receive_str);
		}break;
	}
}
