...
//		dead_limit(&(pidOuter->err[NOW]),DEADBAND);								 //死区限制
		...
		//微分项处理
		pidOuter->dout[NOW]  = (pidOuter->err[NOW]-pidOuter->err[LAST]);//原始微分项
		...
		//微分项进行处理
		pidInner->dout[NOW]  =(pidInner->err[NOW]-pidInner->err[LAST]);
		...
		pidInner->pos_out = (pidInner->pout + pidInner->iout + pidInner->d * pidInner->dout[NOW]);	//位置式PID输出
		//位置式PID输出限幅
